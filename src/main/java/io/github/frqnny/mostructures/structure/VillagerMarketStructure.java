...
public class VillagerMarketStructure extends SpacedStructure<StructurePoolFeatureConfig> {
    public static final Identifier ID = MoStructures.id("villager_market");
    ...
    public static class Start extends MarginedStructureStart<StructurePoolFeatureConfig> {
        ...
        public void init(DynamicRegistryManager registryManager, ChunkGenerator chunkGenerator, StructureManager manager, ChunkPos chunkPos, Biome biome, StructurePoolFeatureConfig config, HeightLimitView heightLimitView) {
            VillagerMarketGenerator.addPieces(registryManager, config, chunkGenerator, manager, new BlockPos(chunkPos.x << 4, 0, chunkPos.z << 4), children, random, heightLimitView);
